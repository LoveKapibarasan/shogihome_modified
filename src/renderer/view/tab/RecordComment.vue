    ...
    <div ref="root" class="full column root" @copy.stop @paste.stop>
      ...
      <div class="row wrap-reverse">
        <div v-if="showBookmark" class="bookmark-area">
          ...
          <button class="list" :disabled="!hasBookmarks" @click="startBookmarkTour">
            <span>{{ t.bookmarkTour }}</span>
          </button>
        </div>
...
const hasBookmarks = computed(() => store.record.bookmarks.length > 0);
...
const startBookmarkTour = () => {
  store.startBookmarkTour();
};
