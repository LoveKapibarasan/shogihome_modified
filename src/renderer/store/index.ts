...
class Store {
  ...
  private _bookmarkTourData?: { bookmarks: string[]; currentIndex: number };
  ...
  startBookmarkTour(): void {
    const bookmarks = this.record.bookmarks;
    if (bookmarks.length === 0) {
      return; // No bookmarks to tour
    }
    
    // Initialize bookmark tour data
    this._bookmarkTourData = {
      bookmarks: bookmarks,
      currentIndex: 0,
    };
    
    // Jump to first bookmark
    this.jumpToBookmark(bookmarks[0]);
    
    // Enter bookmark tour mode
    this._appState = AppState.BOOKMARK_TOUR;
  }

  nextBookmarkInTour(): void {
    if (this._appState !== AppState.BOOKMARK_TOUR || !this._bookmarkTourData) {
      return;
    }
    
    const { currentIndex, bookmarks } = this._bookmarkTourData;
    const nextIndex = currentIndex + 1;
    
    if (nextIndex >= bookmarks.length) {
      // End of tour - exit bookmark tour mode
      this._appState = AppState.NORMAL;
      this._bookmarkTourData = undefined;
      return;
    }
    
    // Jump to next bookmark
    this.jumpToBookmark(bookmarks[nextIndex]);
    
    // Update bookmark tour data
    this._bookmarkTourData.currentIndex = nextIndex;
  }

  previousBookmarkInTour(): void {
    if (this._appState !== AppState.BOOKMARK_TOUR || !this._bookmarkTourData) {
      return;
    }
    ...
    if (prevIndex < 0) {
      return; // Already at first bookmark
    }
    
    // Jump to previous bookmark
    this.jumpToBookmark(bookmarks[prevIndex]);
    
    // Update bookmark tour data
    this._bookmarkTourData.currentIndex = prevIndex;
  }

  exitBookmarkTour(): void {
  ...
  jumpToBookmarkAtIndex(index: number): void {
    if (this._appState === AppState.BOOKMARK_TOUR && this._bookmarkTourData) {
      const { bookmarks } = this._bookmarkTourData;
      if (index >= 0 && index < bookmarks.length) {
        this.jumpToBookmark(bookmarks[index]);
        this._bookmarkTourData.currentIndex = index;
      }
    }
  }
  ...
  showBatchAnalysisDialog(): void {
    if (this.appState === AppState.NORMAL) {
      this._appState = AppState.BATCH_ANALYSIS_DIALOG;
    }
  }
  ...
  destroyModalDialog(): void {
    if (
      ...
      this.appState === AppState.BATCH_ANALYSIS_DIALOG ||
